...
\subsection{MMX}
    Se escribio un codigo en \code{c} donde se multiplicaban dos matrices
    ...
    Este codigo fue dessarrollado en el archivo \code{blocked\_dgemm\_sse.c} que
    se encuentra en el anexo.

\subsection{Bblas}
    Se uso una funcion de la libreria \code{cblas.h} de \code{c}. Esta funcion
    cuya firma es:
    ...
    void cblas_dgemm(
            ...
            const int ldc);
    \end{lstlisting}
    Esta llamada a la rutina \code{cblas\_dgemm} multiplica dos matrices:
    \begin{lstlisting}
