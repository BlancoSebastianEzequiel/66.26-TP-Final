...
\subsection{Multiplicacion de matrices por bloques}
    ...
    \paragraph{Preprocesamiento}
        ...
        \code{(r, c)} de un bloque de la matriz A, tiene el bloque en
...
\subsection{Forma de ejecucion}
    Para el caso de Amdahl multiplicamos dos matrices de \code{10x10}
    con \code{1}, \code{2}, \code{4}, \code{8}, \code{16} y \code{32} threads.\\

    Para el caso de gustafson se usan siempre 4 threads multiplicando dos matrices
    de \code{2x2}, \code{4x4}, \code{8x8}, \code{16x16}, \code{32x32} y
    \code{64x64}\\
